var preferencias = {
    initialize: function() {
        ...
        $('#btnPreferenciasSelecionar').click(function() {
            ...
            myScript.notificacao("Sucesso",
                "Todos os itens foram selecionados!", true);
        });
        //desmarca todos os itens
        $('#btnPreferenciasDesmarcar').click(function() {
            preferencias.listaItens(false);
            $('#itemSelecionadoPreferencias').html("Nenhum item selecionado");
            myScript.notificacao("Sucesso",
                "Todos os itens foram desmarcados!", true);
        });
        ...
        if (storage.preferenciasDistancia) {
            ...
            $('#itemSelecionadoPreferencias').html("");
            for (var i = 0; i < itens.length; i++) {
                for (var j = 0; j < itensStorage.length; j++) {
                    if (itens[i].itmCodigo == itensStorage[j]) {
                        $('#itemSelecionadoPreferencias').append(itens[i].itmNome + ", ");
            ...
            storage.preferenciasDataFormatada = preferencias.dataPreferencia(
                0, 7);
